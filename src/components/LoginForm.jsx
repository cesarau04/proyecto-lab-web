                ...
                .catch((error) => {
                    ...
                    switch (error.code) {
                        case "auth/invalid-email":
                            NotificationManager.error("Bad credentials", "Email or password are wrong", 4000)
                        case "auth/wrong-password":
                            NotificationManager.error("Bad credentials", "Email or password are wrong", 4000)
                        case "auth/user-not-found":
                            NotificationManager.error("No user", "No user was found, try sign up", 4000)
                        default:
                            NotificationManager.error("Unknown", "A unknown error occurred", 4000)
                    }
                    console.log(error);
                })
        }
